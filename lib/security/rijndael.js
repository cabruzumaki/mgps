...
class Rijndael {
    ...
    decrypt(ciphertext) {
        var state = this.packBytes(this.hexToChars(ciphertext));
        this.addRoundKey(state, this.key.slice(this.Nb * this.Nr));
        var i = this.Nr - 1;
        while (i > 0) {
            this.InverseRound(state, this.key.slice(this.Nb * i, this.Nb * (i + 1)));
            i--;
        }
        this.InverseFinalRound(state, this.key.slice(0, this.Nb));
        return this.charsToStr(this.unpackBytes(state));
    }
